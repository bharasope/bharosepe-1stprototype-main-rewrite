...
const AiAgreementGenerator: React.FC<AiAgreementGeneratorProps> = ({
  ...
  const generateAdvancedAgreement = async (data: any) => {
    ...
    const currentUserIsSeller = currentProfile.role === 'Seller';
    
    const contractParams: ContractGenerationParams = {
      transactionType: type,
      amount: Number(details.price) || 0,
      sellerName: currentUserIsSeller ? currentProfile.name : (contact?.name || 'Counterparty'),
      buyerName: currentUserIsSeller ? (contact?.name || 'Counterparty') : currentProfile.name,
      sellerPhone: currentUserIsSeller ? currentProfile.phone : (contact?.phone || 'Phone TBD'),
      buyerPhone: currentUserIsSeller ? (contact?.phone || 'Phone TBD') : currentProfile.phone,
